class NeuralNetwork{
    ...
    static getCopy(newNetwork, network) {
        // console.log(newNetwork, network)
    for (let i = 0; i < newNetwork.levels.length; ++i) {
      for (let j = 0; j < newNetwork.levels[i].weights.length; ++j) {
        for (let k = 0; k < newNetwork.levels[i].weights[j].length; ++k) {
          newNetwork.levels[i].weights[j][k] = network.levels[i].weights[j][k];
        }
      }

      for (let j = 0; j < newNetwork.levels[i].biases.length; ++j) {
        newNetwork.levels[i].biases[j] = network.levels[i].biases[j];
      }
    }
    return newNetwork;
  }
  ...
  fit(input, output){
        let result = this.feedForward(input, this.levels) 
